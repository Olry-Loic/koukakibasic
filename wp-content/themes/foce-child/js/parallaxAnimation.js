 ...
 document.addEventListener("DOMContentLoaded", function () {
  ...
  window.addEventListener("scroll", function () {
    ...
    const logoParallax = document.querySelector(".parallax img");
    logoParallax.style.transform = "translate3d(0, " + +scrollPosition / 3.3 + "px, 0)";

    // mise à jour de la position verticale du logo en fonction de la position de défilement 
    const logo = document.querySelector(".logo");
    logo.style.transform = "translate3d(0, " + -scrollPosition / 1 + "px, 0)";
  });
});
